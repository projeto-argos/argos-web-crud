...
public class GracePeriodDAO {
    ...
    public void insert(GracePeriod gracePeriod) throws SQLException {
        ...
        try (Connection conn = ConnectionFactory.getConnection();
            ...
            if (gracePeriod.getStartDate() != null) {
                stmt.setDate(1, java.sql.Date.valueOf(gracePeriod.getStartDate()));
            } else {
                stmt.setNull(1, java.sql.Types.DATE);
            }

            if (gracePeriod.getEndDate() != null) {
                stmt.setDate(2, java.sql.Date.valueOf(gracePeriod.getEndDate()));
            } else {
    ...
    private GracePeriod mapGracePeriod(ResultSet rs) throws SQLException {
        ...
        java.sql.Date sqlStartDate = rs.getDate("data_inicio_carencia");
        if (sqlStartDate != null) {
            startDate = sqlStartDate.toLocalDate();
        }

        LocalDate endDate = null;
        java.sql.Date sqlEndDate = rs.getDate("data_fim_carencia");
        if (sqlEndDate != null) {
